...
from mlx_audio.utils import apply_quantization, get_model_path
...
class VoxtralRealtime:
    def __init__(
        ...
        streaming_pad_token_id = getattr(
            audio_encoder.special_ids, "streaming_pad", None
        )
        streaming_word_token_id = (
            int(streaming_pad_token_id) + 1
            if streaming_pad_token_id is not None
            else None
        )
        repeat_audio_text_token_id = (
            int(streaming_pad_token_id) + 2
            if streaming_pad_token_id is not None
            else None
        )
    ...
    def stream_transcribe(
        ...
        if strategy == "stable":
            ...
            buffer = StreamingBuffer(
                ...
                transcription_delay_ms=float(
                    audio_config.transcription_delay_ms or 0.0
                ),
                streaming_look_ahead_ms=float(
                    audio_config.streaming_look_ahead_ms or 0.0
                ),
                streaming_look_back_ms=float(
                    audio_config.streaming_look_back_ms or 0.0
                ),
            )
            committed_text = ""
            best_text = ""
            ...
            for chunk in audio_iter:
                ...
                while buffer.read() is not None:
                    ...
                    out = self.transcribe(
                        full_audio, language=language, max_tokens=max_tokens
                    )
                    refresh_count += 1
                    best_text = _prefer_longer_text(best_text, out.text)
            ...
            if accumulated_chunks:
                ...
                if best_text.startswith(committed_text):
                    ...
                    committed_text, delta = _merge_stream_text(
                        committed_text, best_text
                    )
                if delta:
                    yield delta
        ...
        for chunk in audio_iter:
            ...
            if fallback_mode:
                ...
                fallback_best_text = _prefer_longer_text(fallback_best_text, out.text)
            ...
            while (segment := buffer.read()) is not None:
                ...
                if first_iteration:
                    ...
                    if realtime_carry_policy == "lexical":
                        continuation_token = (
                            carry_token if carry_token is not None else last_token
                        )
                    else:
                ...
                if max_tokens > 0:
                    segment_token_budget = min(
                        segment_token_budget, max_tokens - generated_total
                    )
                ...
                if realtime_auto_fallback and _should_fallback_realtime(
                    ...
                    fallback_best_text = _prefer_longer_text(
        ...
        if fallback_mode and accumulated_chunks:
            ...
            should_emit = (
                bool(stripped)
                and len(stripped) >= 16
                and not _should_fallback_realtime(
                    recent_tokens, self._stream_control_token_ids
                )
            )
            if should_emit:
    ...
    def _prepare_audio_conv_features(
        ...
        conv, _ = self.model.audio_encoder.forward_conv_features(
            mx.array(mel), padding_cache=None
        )
        if truncate_left_for_realtime:
    ...
    def _generate_audio_conditioned(
        ...
        audio_cache = self.model.audio_encoder.make_cache()
        ...
        if (
            realtime_expand_single_token
            and audio_embeds_full is not None
            and prompt_len == 1
        ):
        ...
        if audio_embeds_full is not None and effective_prompt_len > int(
            audio_embeds_full.shape[0]
        ):
            audio_position_shift = effective_prompt_len - int(
                audio_embeds_full.shape[0]
            )
        ...
        if use_placeholder_fusion:
            packed_audio = _build_audio_slot_embeddings(
                num_slots=num_audio_slots,
                embed_dim=int(base_prompt_embeds.shape[-1]),
                dtype=base_prompt_embeds.dtype,
                get_slot_embed=_audio_embed_at,
            )
...
def _prefer_longer_text(existing: str, candidate: str) -> str:
