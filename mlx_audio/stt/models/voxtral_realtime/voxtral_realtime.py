...
class Model(VoxtralModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if stream:
            if stream_strategy != "realtime":
                raise ValueError(
                    "VoxtralRealtime supports only stream_strategy='realtime'."
                )
            ...
            def _stream() -> Generator[StreamingResult, None, None]:
                generation_tokens = 0
                for event in runtime.stream_transcribe_tokens(
