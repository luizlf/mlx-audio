...
class CausalConv1d(nn.Module):
    def __init__(
        ...
        self._effective_kernel_size = int(kernel_size)
    ...
    def __call__(self, x: mx.array, padding_state: Optional["Conv1dCacheLayer"] = None) -> mx.array:
        ...
        if padding_state is None:
            n_frames = (
                (x.shape[1] - self._effective_kernel_size + self._padding_total) / self._stride
            ) + 1
            target_length = (
            ...
            x = mx.pad(
                x,
                [(0, 0), (int(self._padding_total), int(extra_padding)), (0, 0)],
            )
        else:
...
class VoxtralRealtimeAudioEncoder(nn.Module):
    def __init__(self, config: AudioConfig):
        ...
        # Effective stride from mel frames to conv feature frames.
        self.total_stride = 2
