...
def _stft(
    ...
    if center:
        # Match torch.stft(center=True) behavior using reflect padding.
        pad = n_fft // 2
...
def compute_log_mel(audio: np.ndarray, config: AudioConfig, center: bool = True) -> np.ndarray:
    ...
    stft = _stft(audio, config.n_fft, config.hop_length, window, center=center)
    magnitudes = np.abs(stft[:, :-1]) ** 2  # drop last frame to match vLLM
