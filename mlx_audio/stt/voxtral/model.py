...
class WhisperCausalMLP(nn.Module):
    def __init__(self, config: AudioConfig):
        super().__init__()
        dim = config.d_model
...
class WhisperCausalEncoderLayer(nn.Module):
    def __init__(self, config: AudioConfig):
        super().__init__()
        self.self_attn = WhisperCausalAttention(config)
        self.self_attn_layer_norm = RMSNorm(config.d_model, eps=1e-5)
        self.mlp = WhisperCausalMLP(config)
...
class WhisperCausalEncoder(nn.Module):
    def __init__(self, config: AudioConfig):
        ...
        self.layers = [WhisperCausalEncoderLayer(config) for _ in range(config.encoder_layers)]
...
class VoxtralModel(nn.Module):
    def __init__(self, text_cfg: TextConfig, audio_cfg: AudioConfig):
        ...
        self.audio_encoder = WhisperCausalEncoder(audio_cfg)
